# Sensitivity analysis that uses the average of top-three FRP instead of peak FRP in the fire classification scheme 
...
start_year = 2008
  ...
  for (year_area in 1:nrow(parameters)) {
    ...
    rate <- lapply(c(start_year:2020), function(treated.year) {
      ...
      df <- readRDS(file.path(outDir, "sens_analysis_low", paste0("analysis_treated", treated.year, "_", area, ".RDS")))
      res <- readRDS(list.files(file.path(outDir, "sens_res_low"),
      ...
      #saveRDS(df.freq.year, file = file.path(outDir, "sens_result_low", paste0("df.freq.year", treated.year, "_", area, ".RDS")))
    ...
    saveRDS(rate.df, file = file.path(outDir, "sens_result_low", paste0(area , "_t", lagged, ".RDS")))
  }
  ...
  for (index in 1:nrow(parameters)) {
    rate <- data.frame(readRDS(file.path(outDir, "sens_result_low",
    ...
    rate$pixels_burn <- sapply(start_year:(start_year + nrow(rate) -1), function(treated.year) {
      df <- readRDS(file.path(outDir, "sens_analysis_low", 
                              paste0("analysis_treated", treated.year, "_", parameters[index,1], ".RDS")))
      return(sum(df$treated))
    })
    
    write.csv(rate, file = file.path(outDir, "sens_result_low",
