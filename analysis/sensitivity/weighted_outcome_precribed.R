# Sensitivity analysis that uses confirmed prescribed only (by FACTS and CAL FIRE) as the exposure
...
for (year_area in 1:nrow(parameters)) {
  ...
  rate <- lapply(c(2008:2020), function(treated.year) {
    ...
    df <- readRDS(file.path(outDir, "prescribed_analysis_low", paste0("analysis_treated", treated.year, "_", area, ".RDS")))
    res <- readRDS(list.files(file.path(outDir, "prescribed_res_low"),
    ...
    saveRDS(df.freq.year, file = file.path(outDir, "prescribed_result_low", paste0("df.freq.year", treated.year, "_", area, ".RDS")))
  ...
  saveRDS(rate.df, file = file.path(outDir, "prescribed_result_low", paste0(area , "_t", lagged, ".RDS")))
}
...
for (index in 1:nrow(parameters)) {
  rate <- data.frame(readRDS(file.path(outDir, "prescribed_result_low", 
  ...
  rate$pixels_burn <- sapply(2008:(2008 + nrow(rate) -1), function(treated.year) {
    df <- readRDS(file.path(outDir, "prescribed_analysis_low", 
                            paste0("analysis_treated", treated.year, "_", parameters[index,1], ".RDS")))
    return(sum(df$treated))
  })
  
  write.csv(rate, file = file.path(outDir, "prescribed_result_low",
