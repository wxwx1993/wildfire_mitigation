...
for (year_area in 1:nrow(parameters)) {
  ...
  rate <- lapply(c(start_year:2020), function(treated.year) {
    ...
    res <- readRDS(list.files(file.path(outDir, "eval_res_low", variable),
    ...
    saveRDS(df.freq.year, file = file.path(outDir, "eval_result_low", variable, paste0("df.freq.year", treated.year, "_", area, ".RDS")))
  ...
  saveRDS(rate.df, file = file.path(outDir, "eval_result_low", variable, paste0(area , "_t", lagged, ".RDS")))
}
...
for (index in 1:nrow(parameters)) {
  rate <- data.frame(readRDS(file.path(outDir, "eval_result_low", variable,
