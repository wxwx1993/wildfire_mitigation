...
#year up to 2020
parameters <- expand.grid(c(2005:2020), c("forests", "savannas", "grasslands"))
# year_area = 1                          
for (year_area in 1:nrow(parameters)) {
  ...
  saveRDS(fire.df_year, file = paste0(outDir, "/analysis_low/fire_class/fire.df", treated.year, "_", area, ".RDS"))
  ...
  saveRDS(df, file = file.path(outDir, "analysis_low", paste0("analysis_treated", treated.year, "_", area, ".RDS")))
}
